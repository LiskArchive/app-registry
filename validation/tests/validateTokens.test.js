...
describe('validateTokens', () => {
	...
	it('should return an array with an error message for invalid baseDenom', async () => {
		...
		const result = await validateTokens(filesToTest);
		expect(result).toEqual([`baseDenom ${invalidConfig.baseDenomWithIncorrectDecimals.tokens[0].baseDenom} is not present in denomUnits with decimals set to 0.`]);
	...
	it('should return an array with an error message for invalid baseDenom', async () => {
		...
		const result = await validateTokens(filesToTest);
		expect(result).toEqual([`baseDenom ${invalidConfig.baseDenomNotInToken.tokens[0].baseDenom} is not present in denomUnits with decimals set to 0.`]);
